 ...
 * <p>
 * Created by 璐从今夜白。 on 2020/7/9 17:37
 * <p>
 * Remark: 全量实时成交列表：/exchange/data/clinch
 */
public class LB2009_BibiClinchTest extends PathData {
    @Autowired
    JsonUtil jsonUtil;
    @Autowired
    ...
    @Test(description = "全量实时成交列表", priority = 0, groups = {ServiceGroup.LB, ProjectGroup.LB})
    public void clinchTest() throws IOException {
        Reporter.log("用例1：全量实时成交列表", true);
        //获取请求路径和url
        ...
        // 设置请求url和路径参数
        getAPI.setUrl(url);
        getAPI.getUrlParams().put("symbol","LBP-USDT");
